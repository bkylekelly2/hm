...
<div class="<?php print $classes; ?>"<?php print $attributes; ?>>
  ...
  <div class="field-items"<?php print $content_attributes; ?>>
    <?php foreach ($items as $delta => $item): ?>
        <div class="field-item <?php print $delta % 2 ? 'odd' : 'even'; ?>"<?php print $item_attributes[$delta]; ?>><a href="/library-search-results?f%5B0%5D=field_ressource_keywords%3A<?php $term_name_sanitized = str_replace('&amp;', '&', $item['#markup']);$term_array = taxonomy_get_term_by_name($term_name_sanitized);
            $term = reset($term_array);
            print $term->tid; ?>"><?php print render($item); ?></a></div>
    <?php endforeach; ?>
  </div>
</div>
